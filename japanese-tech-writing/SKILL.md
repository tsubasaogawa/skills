...
description: 日本語の技術文書・書籍原稿の文章規範。一文一行などの整形、パラグラフライティングによる段落と論証の構成、論証の厳密さ、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除、見出しの付け方、読者への誠実さを定める。技術書の章、記事、解説文を日本語で書くとき、または推敲・リライトするときに使う。
...
規範は多いので、一度にすべてを守ろうとせず、三つのパスに分けて適用する。推敲・リライトの依頼では、執筆パスの規範も点検の観点に加える。

1. **執筆パス**：「整形」「段落と論証の構成」「読み手の負荷の管理」「視点と語り」「見出しの付け方」に従って書く。
2. **論証点検パス**：書き上げたら「論証の厳密さ」「読者への誠実さ」で論理の穴を点検する。
3. **表現点検パス**：最後に「演出の抑制」「LLM っぽい表現の禁止」「冗長の排除」で文面を点検する。
...
- **弱い緩和と称賛**：「〜と言えるだろう」「〜かもしれない」による根拠のない緩和。削るか残すかの判断は「論証の厳密さ」冒頭の基準に従う。「非常に」「極めて」「大いに」 (中身のない強調)
...
- ためらいのある弱い述語 (「有効な対策であり」など) で済ませない。本文内の根拠で確定していることは強く具体的に言い切る (「活用において必須であり」など)。ただし、残すべき弱い述語の判断は「論証の厳密さ」冒頭の基準に従う。語調を整えるための意図的な緩和 (「必須だと言ってもいい」など) は許す。
